...
function filterCourses(products: Array<Product>): Array<Product> {
    return products.filter(product => product.type === 'Course')
}

function getTitles(products: Array<Product>): Array<string> {
...
typeAssert<IsTypeEqual<Product, {
    id: number,
    title: string,
    price: string,
    createdAt: number,
